...
@onready var bee_atlas_scene: Control = %BeeAtlasScene
...
func navigate_forward(new_page: Control):
	var previous_page = navigation_stack[navigation_stack.size() - 1] if navigation_stack.size() else main_menu
...
func on_navigate_back():
	...
	var current_page = navigation_stack[navigation_stack.size() - 1] if navigation_stack.size() else main_menu
